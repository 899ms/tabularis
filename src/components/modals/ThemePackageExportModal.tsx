...
export function ThemePackageExportModal({ isOpen, onClose, theme }: ThemePackageExportModalProps) {
  ...
  return <ThemeDialog isOpen onClose={onClose} busy={busy} icon={<Upload size={20} />} title={t("themePackages.exportPackage")} subtitle={theme.entry.name}
    ...
    <fieldset disabled={busy} className="space-y-4">
      <label className="block space-y-1.5 text-xs font-medium text-secondary">{t("themePackages.packageName")}<input data-autofocus value={name} maxLength={64} onChange={(event) => setName(event.target.value)} className="block w-full bg-base border border-strong rounded-lg px-3 py-2 text-sm font-normal text-primary focus:border-accent-primary focus:outline-none disabled:opacity-50" /></label>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <label className="block space-y-1.5 text-xs font-medium text-secondary">{t("themePackages.version")}<input value={version} onChange={(event) => setVersion(event.target.value)} className="block w-full bg-base border border-strong rounded-lg px-3 py-2 text-sm font-normal text-primary focus:border-accent-primary focus:outline-none disabled:opacity-50" /></label>
        <label className="block space-y-1.5 text-xs font-medium text-secondary">{t("themePackages.minimumRuntime")}<input value={minimum} onChange={(event) => setMinimum(event.target.value)} className="block w-full bg-base border border-strong rounded-lg px-3 py-2 text-sm font-normal text-primary focus:border-accent-primary focus:outline-none disabled:opacity-50" /></label>
      </div>
      <p className="text-xs leading-relaxed text-muted">{t("themePackages.runtimeWarning")}</p>
      <label className="block space-y-1.5 text-xs font-medium text-secondary">{t("themePackages.license")}<textarea value={license} maxLength={256 * 1024} rows={5} onChange={(event) => setLicense(event.target.value)} className="block w-full resize-none bg-base border border-strong rounded-lg px-3 py-2 text-sm font-normal text-primary focus:border-accent-primary focus:outline-none disabled:opacity-50" /></label>
