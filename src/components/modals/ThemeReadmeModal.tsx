...
export function ThemeReadmeModal({ plugin, registryUrl, onClose }: Props) {
  ...
  return <ThemeDialog isOpen onClose={onClose} icon={<Palette size={20} />} title={plugin.name} subtitle={t("themePackages.readme")} widthClass="w-[760px]"
    ...
    {screenshots.length > 0 && <section aria-label={t("themePackages.screenshots")} className="space-y-2"><h3 className="text-sm font-semibold">{t("themePackages.screenshots")}</h3><ul className="space-y-2">{screenshots.map((image, index) => <li key={`${image.url}:${index}`}><button type="button" onClick={() => open(image.url)} className="inline-flex items-center gap-2 text-sm text-accent-primary underline text-left break-words rounded focus-visible:outline focus-visible:outline-accent-primary"><ExternalLink size={14} className="shrink-0" /><span>{image.caption?.trim() || image.alt?.trim() || `${t("themePackages.screenshots")} ${index + 1}`}</span></button></li>)}</ul></section>}
    ...
    {readme?.html ? <div onClick={link} onAuxClick={link} className="text-sm leading-relaxed break-words [&_h1]:text-lg [&_h2]:text-base [&_h1]:font-semibold [&_h2]:font-semibold [&_h2]:mt-4 [&_p]:mb-2 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_pre]:overflow-auto [&_pre]:rounded-lg [&_pre]:p-3 [&_pre]:bg-base [&_a]:text-accent-primary [&_a]:underline" dangerouslySetInnerHTML={{ __html: readme.html }} /> : readme && <p>{t("connectionCatalogue.readmeUnavailable")}</p>}
