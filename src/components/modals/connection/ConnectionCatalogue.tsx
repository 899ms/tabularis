      ...
      <div className="shrink-0 border-b border-default bg-base/60 px-5 pb-3 pt-4 backdrop-blur">
        <div className="relative">
          ...
          <input autoCorrect="off" autoCapitalize="off" autoComplete="off" spellCheck={false}
            ...
            className="w-full rounded-lg border border-default bg-surface-secondary py-2.5 pl-9 pr-9 text-sm text-primary outline-none transition-colors focus:border-accent-primary focus:bg-base"
          />
          {search && (
            <button
              type="button"
        ...
        <div className="mt-2.5 flex flex-wrap items-center gap-1.5">
          {facets.map((f) => {
            ...
            return (
              ...
              >
                {f.label}
                <span className={clsx("ml-1", active ? "text-accent-primary/70" : "text-muted")}>{f.count}</span>
              </button>
            );
          })}
          ...
          <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
            ...
            {hasFilters && (
              <button
                type="button"
                onClick={clearFilters}
                className="cursor-pointer text-xs font-medium text-accent-primary"
              >
