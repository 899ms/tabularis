  ...
  return <ThemeDialog isOpen onClose={onClose} busy={busy} icon={<Palette size={18} />} title={t("themePackages.installTitle")} subtitle={t("themePackages.installSubtitle")} widthClass="w-[560px]" footer={footer}>
    ...
    <div className="flex items-start gap-4">
      ...
      <div className="min-w-0 flex-1">
        <button type="button" onClick={() => void openUrl(registryPageUrl)} title={registryPageUrl} className="inline-flex min-w-0 items-center gap-1 text-left text-base font-semibold text-primary hover:underline underline-offset-4 decoration-accent-primary/60">
    ...
    <div className="flex items-center justify-between rounded-lg border border-default bg-base/60 px-3 py-2 text-[11px]">
      ...
      <button type="button" onClick={() => void openUrl(snapshot.registryUrl)} title={snapshot.registryUrl} className="inline-flex items-center gap-1 font-mono text-primary hover:underline underline-offset-2 truncate ml-2">
