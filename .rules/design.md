...
1. **Colors only through theme tokens.** Use `bg-base`, `bg-elevated`, `bg-surface-*`, `text-primary/secondary/muted/inverse`, `border-default/strong`, `*-accent-primary/secondary/success/warning/error/info` (with `/opacity` for tints) and the `semantic-*` utilities. NEVER write Tailwind palette classes (`text-blue-400`, `bg-red-900/20`, `accent-blue-500`), hex codes, `rgb()`/`hsl()` or inline color literals in components.
2. **Meaning picks the token.** Primary action and links: `accent-primary`. Danger: `accent-error`. Success and run: `accent-success`. Attention: `accent-warning`. Tools, schema changes, themes: `accent-secondary`. Row states and keys in data views: `semantic-modified/new/deleted` and `semantic-pk/fk/index`. Status chips: `src/utils/tones.ts`.
3. **Text over an accent is `text-inverse`, never `text-white`.** Hover brightening is `hover:text-primary`. `text-white` is allowed only over user-picked or brand colors.
...
7. **Verify** with `pnpm lint:theme` (runs inside `pnpm lint`) and by switching themes in Settings, Appearance. Brand and identity colors go in the `ALLOWLIST` of `scripts/check-theme-tokens.mjs` with a reason; UI files never do.
